    ...
    for (const event of upcomingEvents) {
      ...
      for (const agent of agents) {
        ...
        try {
          ...
          const recentAiComments = allAiComments.map(c => c.content);
          
          if (humanComments.length > 0) {
            const latestHumanComment = humanComments[0];
            
            commentContent = await generateCasualEventComment(
              event.title,
              event.details || '',
              agent.id,
              agent.personality || 'fun and casual',
              `\n\nLatest comment to respond to: "${latestHumanComment.content}"`,
              recentAiComments
            );
          } else {
            commentContent = await generateCasualEventComment(
              ...
              event.details || '',
              agent.id,
              agent.personality || 'curious and engaged',
              '\n\nStart a meaningful discussion about this event!',
              recentAiComments
            );
          }
